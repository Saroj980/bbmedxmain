...
import PartyLedgerPrintView from "@/components/accounting/PartyLedgerPrintView";
import { Button } from "@/components/ui/button";
import { Printer, X } from "lucide-react";

export default function PartyLedgerPrintPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to load party ledger for printing", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return (
    <div className="flex justify-center items-center h-screen bg-gray-50">
      <div className="text-gray-500 animate-pulse">Preparing Party Statement...</div>
    </div>
  );

  if (!data) return (
    <div className="flex justify-center items-center h-screen bg-gray-50">
      <div className="text-red-500 font-bold">Failed to load party data.</div>
    </div>
  );
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-[850px] mx-auto bg-white shadow-xl mt-8 print:mt-0 print:shadow-none print:max-w-none print:m-0 print:w-full min-h-[1100px]">
        <PartyLedgerPrintView data={data} systemSettings={systemSettings} />
      </div>
    </div>
  );
}
