...
function PurchaseReportContent() {
  ...
  useEffect(() => {
    Promise.all([
      api.get("/purchases", { params: { ...filters, per_page: 1000 } }),
      api.get("/system-settings"),
    ])
      .then(([purchasesRes, settingsRes]) => {
        setData(purchasesRes.data.data || []);
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={item.id} className="border-b align-top">
                ...
                <td className="p-2 border font-medium">{item.invoice_no}</td>
