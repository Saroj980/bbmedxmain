...
import StockPrintView from "@/components/inventory/StockPrintView";
...
export default function StockPrintPage() {
  ...
  useEffect(() => {
    setMounted(true);
    // Read data passed from the main Stock Status page
    const storedData = sessionStorage.getItem("stock_print_data");
    if (storedData) {
      try {
        setPrintData(JSON.parse(storedData));
      } catch (e) {
        console.error("Failed to parse stock print data", e);
      }
    }
  }, []);
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* Report Container */}
      <div className="max-w-[1100px] mx-auto bg-white shadow-xl mt-8 print:mt-0 print:shadow-none print:max-w-none print:m-0 print:w-full">
        <StockPrintView data={printData} searchQuery="" />
      </div>
