...
export default function SalesReturnDialog({
  ...
  // Track quantities to return per saleItem id
  ...
  const refundSummary = (() => {
    ...
    items.forEach((item: any) => {
      ...
      if (enteredVal > 0 && totalBase > 0) {
        ...
        // Convert entered display value to base units, calculate regular item share
        const enteredValBase = enteredVal * factor;
        const splitRegular = Math.round(enteredValBase * regRatio);

        const financialRatio = regularBase > 0 ? (splitRegular / regularBase) : 0;

        gross += financialRatio * (Number(item.inventory_value || 0) - Number(item.free_carrier_cost || 0));
        vat += financialRatio * Number(item.vat_amount || 0);
        discount += financialRatio * Number(item.discount_amount || 0);
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    const returnItemsPayload = Object.entries(returnQtys)
      .map(([idStr, qtyStr]) => {
        const sale_item_id = Number(idStr);
    ...
    for (const payloadItem of returnItemsPayload) {
      const originalItem = items.find((i: any) => i.id === payloadItem.sale_item_id);
      if (originalItem) {
        const regularBase = Number(originalItem.quantity || 0);
        const freeBase = Number(originalItem.free_qty || 0);
        const totalBase = regularBase + freeBase;
    ...
    try {
      ...
      toast.success("Sales Return (Credit Note) created successfully.");
      setReturnQtys({});
      setRemarks("");
      refresh();
      onClose();
    ...
    <>
      ...
      >
        ...
        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Invoice No: <span className="font-bold text-emerald-600">{sale.invoice_no}</span> ({sale.customer?.name})
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* Header fields: Date & Remarks */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-black uppercase text-gray-400 tracking-wider flex items-center gap-1.5 mb-1.5">
            ...
            <div>
              <label className="text-xs font-black uppercase text-gray-400 tracking-wider flex items-center gap-1.5 mb-1.5">
                <FileText size={14} className="text-[#009966]" />
                Remarks / Reason
              </label>
              <Input
                type="text"
                placeholder="e.g. Damaged products or excess delivery"
                value={remarks}
          ...
          <div>
            ...
            <div className="space-y-4">
              {items.map((item: any) => {
                // Calculate display quantities and remaining quantities
                const regularBase = Number(item.quantity || 0);
                const freeBase = Number(item.free_qty || 0);
                const totalBase = regularBase + freeBase;
                ...
                // Calculate factor (base units per display unit)
                const displayQty = Number(item.unit_breakdown?.[0]?.qty ?? regularBase);
                const factor = displayQty > 0 ? (regularBase / displayQty) : 1;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-white border border-gray-100 p-2.5 rounded-xl text-center text-[10px]">
                      <div>
                        <span className="text-gray-400 block font-semibold uppercase">Sold</span>
